...
- Coverage expectations: use Vitest coverage output (`npm run test:cov`) which runs `vitest --coverage` and writes coverage results to `coverage/`. Use the report to flag untested branches/paths in changed areas and propose focused tests to close gaps.
